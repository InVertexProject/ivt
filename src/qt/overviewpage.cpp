...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::IVT)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("IVT")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeInVertexAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeInVertexAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeInVertexAmount = strAnonymizeInVertexAmount.remove(strAnonymizeInVertexAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeInVertexAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeInVertexAmount * COIN) nMaxToAnonymize = nAnonymizeInVertexAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeInVertexAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeInVertexAmount));
        strAnonymizeInVertexAmount = strAnonymizeInVertexAmount.remove(strAnonymizeInVertexAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeInVertexAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
