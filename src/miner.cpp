...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("InVertexMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("InVertexMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("InVertexMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("invertex-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running InVertexMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
